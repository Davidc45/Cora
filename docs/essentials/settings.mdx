...
<Note>
  **Cora repository:** product branding for **this Mintlify site** lives in **`docs/docs.json`** at the repo root (e.g. `name`, `description`, `colors`, `fonts`, `appearance`, `navigation`). The **Next.js app** under `src/` uses separate CSS and layout — do not confuse the two.
</Note>

Every Mintlify site needs a `docs.json` file with the core configuration settings. In this monorepo that file is **`docs/docs.json`**. Learn more about the [properties](#properties) below.
